...
function Eventos() {
    $('#btnSalvarMegaSena').on('click', function (e) {
        if (ValidaCampos('.card-mega-sena')) {
            var numerosSorteados = "";
            numerosSorteados += $('#txtNumero1').val() + '-' +
                $('#txtNumero2').val() + '-' +
                $('#txtNumero3').val() + '-' +
                $('#txtNumero4').val() + '-' +
                $('#txtNumero5').val() + '-' +
            ...
            var megaSena = {
                NumConcurso: $('#txtNumeroConcurso').val(),
                DatConcurso: $('#txtDataConcurso').val(),
                NumSorteados: numerosSorteados,
                QtdeGanhadoresSena: $('#txtQtdeGanhadoresSena').val(),
                ValorGanhadoresSena: $('#txtValorGanhadoresSena').val(),
                QtdeGanhadoresQuina: $('#txtQtdeGanhadoresQuina').val(),
                ValorGanhadoresQuina: $('#txtValorGanhadoresQuina').val(),
                QtdeGanhadoresQuadra: $('#txtQtdeGanhadoresQuadra').val(),
                ValorGanhadoresQuadra: $('#txtValorGanhadoresQuadra').val(),
            ...
            $.ajax({
                ...
                success: function (resultado) {
                    if (!resultado.sucesso)
                        ...
                        alert('Salvo com sucesso - MEGA-SENA')
                },
...
function ValidaCampos(classeCard) {
    var valores = $(classeCard).find('[id*=txt]').map(function (idx, elem) {
        return $(elem).val();
    }).get();
        ...
        success: function (retorno) {
            if (retorno.sucesso) {
                if (retorno.tipo == 1) {
                    ...
                    $('#txtQtdeGanhadoresQuina').val(retorno.resultado.QtdeGanhadoresQuina);
                    $('#txtValorGanhadoresQuina').val(FormataValoresCom1CasaDecimal(retorno.resultado.ValorGanhadoresQuina));
